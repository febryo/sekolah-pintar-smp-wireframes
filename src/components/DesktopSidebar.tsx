...
const DesktopSidebar: React.FC<DesktopSidebarProps> = ({ 
  ...
  const navItems = [
    ...
    { id: 'schedule', label: 'Jadwal', icon: <Calendar className="h-5 w-5" /> },
        ...
        <SidebarFooter className="px-4 pb-6">
          <div className="pt-2 border-t">
            <div className="flex items-center mb-4 px-2 py-2">
              ...
              <div>
                <p className="text-sm font-medium">{role === 'admin' ? 'Admin Sekolah' : 'Guru'}</p>
                <p className="text-xs text-muted-foreground">{role === 'admin' ? 'admin@sekolah.com' : 'guru@sekolah.com'}</p>
              </div>
            </div>
            <Button 
              variant="outline"
